...
<div class="fieldcontain ${hasErrors(bean: productInstance, field: 'manufacturer', 'error')} required">
	...
	<g:select id="manufacturer" name="manufacturer.id" from="${criteriaquerydemo.Company.list()}" optionKey="id" required="" value="${productInstance?.manufacturer?.id}" class="many-to-one"/>
</div>
