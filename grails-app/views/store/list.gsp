...
<%@ page import="criteriaquerydemo.Store" %>
<!DOCTYPE html>
<html>
	<head>
			...
			<table>
				...
				<g:each in="${storeInstanceList}" status="i" var="storeInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${storeInstance.id}">${fieldValue(bean: storeInstance, field: "city")}</g:link></td>
					
						<td>${fieldValue(bean: storeInstance, field: "name")}</td>
					
						<td>${fieldValue(bean: storeInstance, field: "state")}</td>
