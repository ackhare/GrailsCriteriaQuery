<%@ page import="criteriaquerydemo.Transaction" %>
...
<div class="fieldcontain ${hasErrors(bean: transactionInstance, field: 'product', 'error')} required">
	...
	<g:select id="product" name="product.id" from="${criteriaquerydemo.Product.list()}" optionKey="id" required="" value="${transactionInstance?.product?.id}" class="many-to-one"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: transactionInstance, field: 'store', 'error')} required">
	...
	<g:select id="store" name="store.id" from="${criteriaquerydemo.Store.list()}" optionKey="id" required="" value="${transactionInstance?.store?.id}" class="many-to-one"/>
</div>
